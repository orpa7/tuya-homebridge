  ...
  refreshAccessoryServiceIfNeed(statusArr, isRefresh) {
    ...
    for (var statusMap of statusArr) {
      ...
      if (statusMap.code === 'temp_current' || statusMap.code === 'temp_current_f') {
        ...
        this.normalAsync(Characteristic.HeatingThresholdTemperature, this.temperatureMap.value/10, {
          minValue: 2,
          maxValue: 28,
          minStep: 1
        })
        ...
        {
          this.normalAsync(Characteristic.TargetHeatingCoolingState, 3,{validValues: [Characteristic.TargetHeatingCoolingState.HEAT,3]})
        }
        else if (this.modeMap.value == "manual"  ) {
          this.normalAsync(Characteristic.TargetHeatingCoolingState, 1,{validValues: [Characteristic.TargetHeatingCoolingState.HEAT,3]})
        }
        else  {
          this.normalAsync(Characteristic.TargetHeatingCoolingState, 3,{validValues: [Characteristic.TargetHeatingCoolingState.HEAT,3]})
        }
